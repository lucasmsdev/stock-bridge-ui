...
export default function Dashboard() {
  ...
  const { calculateFees, isLoading: feesLoading } = useMarketplaceFees();
  ...
  const handleGenerateDemoData = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Erro ao gerar dados", description: error.message || "Tente novamente" });
    } finally {
  ...
  const handleDeleteDemoData = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Erro ao apagar dados", description: error.message || "Tente novamente" });
    } finally {
  ...
  const calculateMonthlyExpense = (expense: Expense): number => {
    ...
    switch (expense.recurrence) {
      ...
      case 'one-time':
        ...
        if (startMonth === now.getMonth() && startYear === now.getFullYear()) return expense.amount;
        return 0;
  ...
  const loadAllData = useCallback(async (filters: DashboardFiltersState) => {
    ...
    try {
      ...
      const [productsRes, todayOrdersRes, periodOrdersRes, allOrdersRes, expensesRes] = await Promise.all([
        ...
        supabase.from('expenses').select('id, amount, category, recurrence, is_active, start_date, end_date')
      ...
      products.forEach(p => {
        if (p.selling_price && p.cost_price) {
          productCostMap.set(Number(p.selling_price), Number(p.cost_price));
        }
      });

      // Calculate real profit per order using marketplace fees
      const calculateOrderProfit = (order: any) => {
        const revenue = Number(order.total_value || 0);
        const platform = (order.platform || '').toLowerCase();
        const fees = calculateFees(platform, revenue);
        
        // Calculate product cost from items
        let productCost = 0;
        const items = (order.items as any[]) || [];
        items.forEach(item => {
          const cost = productCostMap.get(Number(item.unit_price));
          if (cost) {
            productCost += cost * (item.quantity || 1);
          }
        });
        // Fallback: estimate 55% if no costs found
        ...
        return {
          revenue,
          marketplaceFees: fees.commissionAmount + fees.paymentFeeAmount + fees.fixedFeeAmount,
          taxAmount: fees.taxAmount,
          productCost,
          grossProfit: revenue - (fees.commissionAmount + fees.paymentFeeAmount + fees.fixedFeeAmount) - fees.taxAmount - productCost,
        };
      };
      ...
      periodOrders.forEach(order => {
        ...
        const revenue = Number(order.total_value || 0);
        if (salesByDay[date] !== undefined) salesByDay[date] += revenue;
        periodTotalProfit += calculateOrderProfit(order).grossProfit;
      });
      ...
      // Monthly evolution with real fees
      ...
      allOrders.forEach(order => {
        ...
        if (monthlyData[key] !== undefined) {
          const orderResult = calculateOrderProfit(order);
          monthlyData[key].revenue += orderResult.revenue;
          monthlyData[key].profit += orderResult.grossProfit;
        }
      });
      ...
      setHasData(totalProducts > 0 || todayOrdersCount > 0 || periodRevenue > 0);
  ...
  const handleClearFilters = () => {
    const defaultFilters: DashboardFiltersState = { marketplace: "all", period: "30days" };
  ...
  const getPeriodLabel = () => {
    const labels: Record<string, string> = {
      today: "Hoje", "7days": "Últimos 7 dias", "30days": "Últimos 30 dias",
      "90days": "Últimos 90 dias", "180days": "Últimos 180 dias",
      this_month: "Este mês", last_month: "Mês passado", custom: "Período personalizado",
    };
  ...
  // Calculate net profit using real marketplace fees from monthly data
  const currentMonthData = monthlyProfitData[monthlyProfitData.length - 1];
    ...
    <div className="space-y-4 md:space-y-6 animate-fade-in">
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
          <div>
            ...
            <p className="text-muted-foreground font-body">Visão geral das suas vendas e métricas</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <Badge variant="outline" className="capitalize">Plano {currentPlan}</Badge>
            <Button variant="outline" size="sm" onClick={handleGenerateDemoData} disabled={isGeneratingDemo || isDeletingDemo} className="gap-1.5">
              {isGeneratingDemo ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
              {isGeneratingDemo ? 'Gerando...' : 'Gerar Demo'}
            </Button>
            {hasData && (
              <Button variant="destructive" size="sm" onClick={handleDeleteDemoData} disabled={isGeneratingDemo || isDeletingDemo} className="gap-1.5">
                {isDeletingDemo ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
      ...
      <Tabs defaultValue="vendas" className="w-full">
        <TabsList className="grid w-full max-w-lg grid-cols-3">
          <TabsTrigger value="vendas" className="gap-2"><DollarSign className="h-4 w-4" />Vendas</TabsTrigger>
          <TabsTrigger value="ads" className="gap-2"><Megaphone className="h-4 w-4" />Ads</TabsTrigger>
          <TabsTrigger value="roi" className="gap-2"><PieChart className="h-4 w-4" />ROI</TabsTrigger>
        </TabsList>

        <TabsContent value="vendas" className="mt-6 space-y-4 md:space-y-6">
          <DashboardFilters onApply={handleApplyFilters} onClear={handleClearFilters} isLoading={isLoading} />

          {(activeFilters.marketplace !== "all" || activeFilters.period !== "30days") && (
            <div className="flex flex-wrap gap-2">
              {activeFilters.marketplace !== "all" && (
                <Badge variant="secondary" className="gap-1"><Store className="h-3 w-3" />{getMarketplaceLabel()}</Badge>
              )}
              <Badge variant="secondary" className="gap-1"><Calendar className="h-3 w-3" />{getPeriodLabel()}</Badge>
            </div>
          )}
          ...
          {!hasData && (
            <div className="p-8 text-center border border-dashed border-border rounded-lg bg-muted/20">
              ...
              <p className="text-sm text-muted-foreground mb-4">Clique em "Gerar Demo" para criar dados fictícios para visualização.</p>
            </div>
          )}
          ...
          {hasData && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 lg:gap-6">
                {metrics.map((metric) => (
                  <Card key={metric.title} className="shadow-soft hover:shadow-medium transition-all duration-200 hover:scale-[1.02] hover:-translate-y-1 cursor-pointer">
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground font-body mt-1">
                        <span className={metric.trend.startsWith('+') ? 'text-success' : 'text-destructive'}>{metric.trend}</span> vs ontem
                      </p>
                    </CardContent>
                  </Card>
                ))}
                ...
                <CardContent className="pt-6">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className={`h-14 w-14 rounded-full flex items-center justify-center ${netProfit >= 0 ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Faturamento ({formatCurrency(dashboardData.periodRevenue)}) - Taxas reais por marketplace
                        </p>
                      </div>
                    </div>
                    <div className="flex flex-col md:items-end gap-2">
                      <div className="flex items-center gap-4">
                        <div className="text-center">
                          ...
                          <p className={`text-lg font-bold ${netMargin >= 10 ? 'text-emerald-600' : 'text-amber-600'}`}>{netMargin.toFixed(1)}%</p>
                        </div>
                        <div className="text-center">
                          <p className="text-xs text-muted-foreground">Despesas/Mês</p>
                          <p className="text-lg font-bold text-amber-600">{formatCurrency(totalMonthlyExpenses)}</p>
                        </div>
                      </div>
                      <Link to="/app/expenses" className="text-xs text-primary hover:underline flex items-center gap-1">
                        <Receipt className="h-3 w-3" />Gerenciar despesas
                      </Link>
                    </div>
                  </div>
                    ...
                    <div className="h-[300px] w-full">
                      {dashboardData.salesByDay.some(d => d.revenue > 0) ? (
                        <ChartContainer config={chartConfig} className="h-full w-full">
                          <BarChart data={dashboardData.salesByDay.map(item => ({ ...item, displayDate: formatDate(item.date) }))} margin={{ top: 20, right: 20, left: 0, bottom: 40 }} barCategoryGap="25%">
                            <XAxis dataKey="displayDate" tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                            <YAxis hide />
                            <ChartTooltip content={<ChartTooltipContent formatter={(value) => [formatCurrency(Number(value)), "Receita"]} />} />
                    ...
                    <div className="h-[300px] w-full">
                      {monthlyProfitData.length > 0 ? (
                        <ChartContainer config={profitChartConfig} className="h-full w-full">
                          <AreaChart data={monthlyProfitData} margin={{ top: 20, right: 20, left: 0, bottom: 40 }}>
                            <defs>
                            ...
                            <YAxis tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} tickLine={false} axisLine={false} tickFormatter={(value) => `R$${(value / 1000).toFixed(0)}k`} width={50} />
                                ...
                                return (
                                  <div className="rounded-lg border bg-background p-3 shadow-md">
                                    ...
                                    <div className="space-y-1 text-sm">
                                      <p className="text-muted-foreground">Faturamento: <span className="font-medium text-foreground">{formatCurrency(data.revenue)}</span></p>
                                      <p className="text-muted-foreground">Despesas: <span className="font-medium text-red-500">-{formatCurrency(data.expenses)}</span></p>
                            ...
                            <Area type="monotone" dataKey="netProfit" stroke="hsl(142.1 76.2% 36.3%)" strokeWidth={2} fill="url(#profitGradient)" />
                          </AreaChart>
                        </ChartContainer>
                      ) : (
