...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const todayRevenue = todayData?.reduce((sum, order) => sum + Number(order.total_value), 0) || 0;
    ...
    const salesLast7Days = Object.entries(salesByDate).map(([date, revenue]) => ({
      date,
    ...
    console.error('Error in get-dashboard-metrics function:', error);
    return new Response(
